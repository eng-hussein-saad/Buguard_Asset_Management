...
**Independent Test**: Authenticate as analyst and admin users, create an asset without providing organization ownership, read it back, update allowed fields, and verify deletion or archival is available only to an admin.
...
3. **Given** an admin is authenticated and owns an existing organization asset, **When** they delete or archive the asset, **Then** the asset is no longer available as an active organization asset according to the project's chosen deletion behavior.
4. **Given** a viewer is authenticated, **When** they attempt to create, update, delete, or archive an asset, **Then** the action is rejected with a structured forbidden error.
...
2. **Given** a user references an asset identifier owned by another organization, **When** they read, update, delete, or archive that asset, **Then** the system responds as if the asset is unavailable to that user.
3. **Given** a client submits organization ownership in an asset create or update payload, **When** the request is processed, **Then** the supplied ownership is ignored or rejected and cannot change the authenticated organization scope.
...
- Page numbers below 1, page sizes below 1, and page sizes above the allowed maximum must be rejected or normalized consistently and documented.
...
- Duplicate assets within the same organization must be handled consistently with the Phase 2 uniqueness rule.
...
- Missing, deleted, archived, or cross-organization assets must produce predictable domain errors according to the selected deletion behavior.
...
- **FR-001**: System MUST provide authenticated asset create, list, detail, update, and delete-or-archive operations.
- **FR-002**: System MUST derive asset ownership from the authenticated user's organization context for every asset create, read, update, list, delete, and archive operation.
- **FR-003**: System MUST NOT accept or trust organization ownership supplied by clients in asset create or update requests.
...
- **FR-007**: System MUST allow only admin users to delete or archive assets owned by their organization.
...
- **FR-031**: System MUST keep all asset list, detail, update, and delete-or-archive queries scoped to the authenticated user's organization.
...
- **FR-034**: System MUST update README notes if Phase 3 changes how users should create, query, update, delete, archive, or test assets.
...
- **SC-001**: 100% of Phase 3 asset create, list, detail, update, and delete-or-archive tests pass for the roles allowed to perform each action.
- **SC-002**: 100% of unauthorized or unauthenticated Phase 3 asset operation tests are rejected.
- **SC-003**: 100% of tenant isolation tests confirm users cannot list, read, update, delete, archive, or infer assets owned by another organization.
- **SC-004**: Asset create and update payload tests confirm clients cannot set or change organization ownership.
...
- **SC-007**: Pagination tests verify default page 1, default page size 20, maximum page size 100, empty result behavior, and invalid pagination handling.
...
- Delete behavior may be implemented as hard delete or archive as long as admin-only access, tenant isolation, response behavior, and documentation are consistent with the implementation plan.
